...
async def test_ler_categoria(client, categoria_teste):
    ...
    model_categoria_criada = s_Categorias_out.model_validate(categoria_teste).model_dump(mode='json')
...
async def test_atualizar_categoria_duplicada(client, categoria_teste, async_session):
    ...
    response_atualizacao = client.patch(f'/categorias/{categoria_teste.id}', json=dados_atualizacao)
    await async_session.refresh(categoria_teste)
