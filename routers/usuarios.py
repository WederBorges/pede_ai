...
async def read_user(id_user:int,session = Depends(async_get_session)): 
    ...
    if not user:
        raise HTTPException(
            HTTPStatus.NOT_FOUND,
            detail='Não existe este usuário'
        )
    return user

@router.patch('/{id_user}', status_code=HTTPStatus.OK, response_model=s_Usuario_Update_out)
async def update_user(id_user:int, dados: s_Usuario_update_, session=Depends(async_get_session)):
